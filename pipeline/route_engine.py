...
def build_route(sections_map: dict, price: float, direction: str, tf_hours: float = 4.0,
    ...
    # ═══════════════════════════════════════════
    # ШАГ 7b: Пересчёт слома по СЦЕНАРИЮ МАРШРУТА
    # ═══════════════════════════════════════════
    # Слом = уровень, пробой которого инвалидирует СЦЕНАРИЙ (маршрут).
    # Маршрут вверх → слом НИЖЕ цены (пробой вниз ломает сценарий).
    # Маршрут вниз → слом ВЫШЕ цены.
    # Сценарий может расходиться со старшим трендом S01 (например,
    # накопление фазы E при формально нисходящем тренде) — слом всегда
    # ставится против ФАКТИЧЕСКОГО направления маршрута.
    SLAM_OFFSET_PCT = 0.005  # 0.5% запас за экстремумом маршрута
    if len(route) >= 2:
        _route_final_up = route[-1]["price"] >= price
        slam_should_be_below = _route_final_up
        slam_should_be_above = not _route_final_up
